...
export async function addLocation(creator, title, description, latitude, longitude) {
    let queryURL = url.concat(`/addlocation`);
    let requestBody = {username: creator, title: title, description: description, latitude: latitude, longitude: longitude};
    try {
        const response = await fetch(queryURL, {
            method: 'POST',
            headers: {
